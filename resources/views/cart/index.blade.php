...
@section('content')
    ...
    <section>
        ...
        @if ($items->isEmpty())
            ...
            <!-- Message -->
            <div class="py-3 px-6 rounded-lg bg-pink text-white">Корзина пустая</div>
                            ...
                            <td scope="row" class="py-4 px-4 md:px-6 rounded-l-2xl bg-card">
                                <div class="flex flex-col lg:flex-row min-w-[200px] gap-2 lg:gap-6">
                                    <div class="shrink-0 overflow-hidden w-[64px] lg:w-[84px] h-[64px] lg:h-[84px] rounded-2xl">
                                        <img src="{{ $item->product->makeThumbnail('70x70') }}" class="object-cover w-full h-full" alt="{{ $item->product->title }}">
                                    </div>
                                    <div class="py-3">
